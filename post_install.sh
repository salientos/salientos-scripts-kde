...
# ensure sddm has acces to .face icon
[[ -e /usr/bin/setfacl ]] && setfacl -m u:sddm:x /home/${n_user}/
[[ -e /usr/bin/setfacl ]] && setfacl -m u:sddm:r /home/${n_user}/.face.icon

# remove autologin for liveuser
[[ -f /etc/sddm.conf.d/autologin.conf ]] && rm -f /etc/sddm.conf.d/autologin.conf

# ensure our new user is set in sddm.conf
[[ -f /usr/bin/sed ]] && sed -i "s|liveuser|${n_user}|g" /etc/sddm.conf
...
[[ -e /usr/bin/ln ]] && ln -s /home/${n_user}/.config/Trolltech.conf /etc/xdg/Trolltech.conf
