...
_remove_unwanted_packages() {
    local _packages_to_remove=(
        ...
        'partimage'
        'salientos-calamares'
        'salientos-calamares-kde-config'
        'syslinux'
        'termite-terminfo'
        'testdisk'
        'yaml-cpp'
...
_perform_various_stuff() {
    ...
    #-------------------------------------------------------------------------------------
    # Fix Up SDDM Specific Settings ------------------------------------------------------
    #-------------------------------------------------------------------------------------

    [[ -f /etc/sddm.conf.d/autologin.conf ]] && rm -f /etc/sddm.conf.d/autologin.conf
    [[ -f /etc/sddm.conf.d/kde_settings.conf ]] && rm -f /etc/sddm.conf.d/kde_settings.conf

    # give sddm permission to read .face.icon
    [[ -e /usr/bin/setfacl ]] && setfacl -m u:sddm:x /home/${new_user}/
    [[ -e /usr/bin/setfacl ]] && setfacl -m u:sddm:r /home/${new_user}/.face.icon

    # check exists and remove
    [[ -f /etc/xdg/Trolltech.conf ]] && rm -f /etc/xdg/Trolltech.conf

    # rebuild symlink to fix root theme
    [[ -e /usr/bin/ln ]] && ln -s /home/${new_user}/.config/Trolltech.conf /etc/xdg/Trolltech.conf
